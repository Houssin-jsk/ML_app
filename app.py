...
# === ⬇️ Fonction pour télécharger fichier depuis Google Drive
def download_from_google_drive(file_id):
    ...
    if response.status_code == 200:
        ...
        st.error("⚠️ Erreur de téléchargement depuis Google Drive.")
        st.stop()

# === 📦 Téléchargement du modèle
file_id = "1Iu5S6LzuMr9ePxy0QiY2geO_yohAILsa"  # 👉 Remplace bih l vrai ID
local_model_path = download_from_google_drive(file_id)
...
try:
    with open(local_model_path, "rb") as f:
        ...
        if isinstance(data, dict) and "model" in data and "features" in data:
            ...
            st.error("⚠️ Le fichier ne contient pas les clés 'model' et 'features'.")
            st.stop()
...
with tab2:
    ...
    sns.heatmap(corr_matrix, cmap='coolwarm', annot=True, fmt=".2f", vmin=-1, vmax=1, ax=ax_corr)
